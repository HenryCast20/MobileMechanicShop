...
const getCars = async (req, res) => {
    ...
    //ADD A CAR to logged in user
    const addCar = async (req, res) => {
        try {
            const customerId = req.user.customer_id;
            const {make, model, year_produced, vin,odometer,license_plate} = req.body;

            const [result] = await db.query(
            'INSERT INTO vehicles (customer_id, make, model, year, vin, odometer, license_plate) VALUES (?, ?, ?, ?, ?)', [customerId, make, model, year_produced, vin,odometer,license_plate]
            );
